...
/**
 * BeamGridBackground – Animated grid with glowing beams
 * Ported from Lightswind UI (TypeScript → JSX)
 *
 * Props:
 *   gridSize          – cell size in px (default 40)
 *   gridColor         – grid line color (default "#e5e7eb")
 *   darkGridColor     – grid line color in dark mode (default "#27272a")
 *   beamColor         – beam color (default "rgba(0, 180, 255, 0.8)")
 *   darkBeamColor     – beam color in dark mode
 *   beamSpeed         – base speed (default 0.1)
 *   beamThickness     – beam stroke width (default 3)
 *   beamGlow          – enable glow (default true)
 *   glowIntensity     – shadowBlur value (default 50)
 *   beamCount         – primary beams (default 8)
 *   extraBeamCount    – secondary beams (default 3)
 *   idleSpeed         – speed multiplier when idle (default 1.15)
 *   interactive       – mouse highlight (default true)
 *   asBackground      – absolute position (default true)
 *   showFade          – radial fade mask (default true)
 *   fadeIntensity     – fade % (default 20)
      ...
      allBeams.forEach((beam) => {
        ...
        if (beam.dir === "x") {
          ...
          beam.offset += idle
            ? beam.speed * idleSpeed * 60 * 0.016
            : beam.speed * 60 * 0.016;
          if (beam.offset > rect.height + beamLength)
