import { CheckCircle, XCircle, FileText, MessageSquare, Pill } from "lucide-react";
              ...
              {appointments.map((apt) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3.5 pr-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          {patientId && (
                            <p className="text-[10px] text-gray-400">{patientId}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 pr-4">
                      {apt.priority && (
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold capitalize ${PRIORITY_BADGE[apt.priority] || PRIORITY_BADGE.low}`}>
                          {apt.priority}
                        </span>
                      )}
                    </td>
                    ...
                    {/* Status */}
                    <td className="py-3.5 pr-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold capitalize ${STATUS_BADGE[apt.status] || ""}`}>
                        {apt.status}
                      </span>
                    </td>
...
function ActionIcon({ icon, title, color, onClick }) {
  return (
    <button
      title={title}
      onClick={onClick}
      className={`p-1.5 rounded-lg transition-colors active:scale-95 ${COLOR_CLASSES[color] || COLOR_CLASSES.gray}`}
    >
      {icon}
    </button>
  );
