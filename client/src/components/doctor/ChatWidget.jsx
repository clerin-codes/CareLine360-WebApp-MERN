...
export default function ChatWidget({ appointment, onClose }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-80 md:w-96 flex flex-col bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden"
      style={{ height: "520px" }}>
      ...
      <div className="bg-teal-600 px-4 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="relative">
            ...
            <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-teal-600 ${connected ? "bg-green-400" : "bg-gray-400"}`} />
          </div>
          <div>
            <p className="text-white font-semibold text-sm leading-tight">{patientName}</p>
            <p className="text-teal-200 text-xs">
              {connected ? "Online" : "Connecting…"} ·{" "}
              {new Date(appointment.date).toLocaleDateString("en-GB")} {appointment.time}
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 min-h-0">
        {loading ? (
          ...
          messages.map((msg) => {
            const isMe = msg.senderId === myUserId || msg.senderId?.toString() === myUserId;
            return (
              <div key={msg._id || msg.createdAt} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[78%] group relative`}>
                  ...
                  <div className={`flex items-center gap-1 mt-0.5 ${isMe ? "justify-end" : "justify-start"}`}>
                    ...
                    {isMe && (
                      <span className={`text-[10px] ${msg.isRead ? "text-teal-500" : "text-gray-400"}`}>
                        {msg.isRead ? "✓✓" : "✓"}
                      </span>
                    )}
                  </div>
