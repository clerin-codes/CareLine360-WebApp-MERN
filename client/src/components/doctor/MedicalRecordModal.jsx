...
const VITALS_FIELDS = [
  ...
  { key: "temperature", label: "Temperature (°C)", placeholder: "37.0", type: "number" },
  { key: "oxygenSaturation", label: "O₂ Saturation (%)", placeholder: "98", type: "number" },
...
export default function MedicalRecordModal({ patientId, appointmentId, onClose, onSaved }) {
  ...
  const setVital = (key) => (e) => setForm((p) => ({ ...p, vitals: { ...p.vitals, [key]: e.target.value } }));
  ...
  const addMed = () => setForm((p) => ({ ...p, prescriptions: [...p.prescriptions, emptyMed()] }));
  const removeMed = (i) => setForm((p) => ({ ...p, prescriptions: p.prescriptions.filter((_, idx) => idx !== i) }));
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-2xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="font-semibold text-gray-800 dark:text-gray-100">Add Medical Record</h2>
        ...
        <div className="overflow-y-auto p-6 space-y-5 flex-1">
          ...
          <Textarea label="Chief Complaint" value={form.chiefComplaint} onChange={setField("chiefComplaint")} placeholder="Primary reason for visit" />
          <Textarea label="Diagnosis" value={form.diagnosis} onChange={setField("diagnosis")} placeholder="Clinical diagnosis" />
          <Textarea label="Notes" value={form.notes} onChange={setField("notes")} placeholder="Additional clinical notes" />

          {/* Vitals */}
          <div>
            <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">Vitals</h3>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={addMed} className="text-teal-600 text-sm hover:underline">+ Add</button>
            </div>
              ...
              {form.prescriptions.map((med, i) => (
                <div key={i} className="p-4 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                  <div className="flex justify-between mb-3">
                    <p className="text-xs font-medium text-gray-500">Medication #{i + 1}</p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["medicine", "dosage", "frequency", "duration"].map((k) => (
                      <input
                        key={k}
                        value={med[k]}
                        onChange={setMed(i, k)}
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-700">
          <button onClick={onClose} className="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">Cancel</button>
          <button
            onClick={handleSave}
            disabled={saving}
